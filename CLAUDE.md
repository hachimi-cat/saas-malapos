...
| `cli/` | Commander-based CLI. `auth login/whoami/logout` ship; session via `@forjio/sdk` `Session` at `~/.malapos/credentials` (INI, AWS-style profiles — see `src/lib/context.ts`). |
